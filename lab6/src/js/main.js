...
function validateInput(inputId) {
    ...
    if (isNaN(value) || value <= 0) {
        document.getElementById("error-message").textContent = "Будь ласка, введіть коректні дані!";
        inputElement.value = ''; // очистка поля при введенні некоректних даних
    } else {
...
function calculateRectangle() {
    ...
    if (isNaN(length) || isNaN(width) || length <= 0 || width <= 0) {
        return; // при все ще некоректних даних не робимо розрахунків.
    }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    calculateButton.addEventListener("click", function() {
        ...
        if (isNaN(birthDate)) {
            ...
            // назва дня тижня на основі результату
